...
static inline int sgn(long double a) { return (a > 0) - (a < 0); }
...
static inline V2 to_2d(const V3 &p, int t) { return V2{p.v[(t + 1) % 3], p.v[(t + 2) % 3]}; }
...
static bool old_cut(const V3 &tri0, const V3 &tri1, const V3 &tri2, const V3 &bmin, const V3 &bmax) {
  ...
  if (cut) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      mp[0] = to_2d(bmin, i);
      mp[1] = to_2d(bmax, i);
      mp[2].x = mp[0].x; mp[2].y = mp[1].y;
      mp[3].x = mp[1].x; mp[3].y = mp[0].y;
      for (int j = 0; j < 3; j++) {
...
static Cache build(const V3 &t0, const V3 &t1, const V3 &t2) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < 3; j++)
      c.ori[i][j] = orient_2d(c.tri2d[i][(j + 2) % 3], c.tri2d[i][j % 3], c.tri2d[i][(j + 1) % 3]);
  }
  return c;
}
...
int main() {
  ...
  auto run = [&](bool ints, long n) -> long {
    ...
    for (long k = 0; k < n; k++) {
      ...
      auto g = [&](V3 &p) { for (int c = 0; c < 3; c++) p.v[c] = ints ? (double)di(rng) : dr(rng); };
      g(t0); g(t1); g(t2); g(ba); g(bb);
      for (int c = 0; c < 3; c++) {
    ...
    printf("regime ints=%d: n=%ld mismatches=%ld (accept-rate=%.1f%%)\n", ints, n, mismatch,
           100.0 * trues / n);
    return mismatch;
  };
  long m = 0;
